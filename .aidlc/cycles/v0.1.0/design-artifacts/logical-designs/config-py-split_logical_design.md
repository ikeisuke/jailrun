...
```
lib/
├── config.py          # TOML解析・設定値API（エントリポイントなし）
...
削除する。config.pyはライブラリモジュールとしてのみ使用。
...
### 3. config.py 直接実行
既存テストにはconfig.pyを直接呼び出すテストが存在しない。直接呼び出しの後方互換テストは本Unitのスコープ外（`python3 lib/config.py <subcommand>`は非推奨化）。
