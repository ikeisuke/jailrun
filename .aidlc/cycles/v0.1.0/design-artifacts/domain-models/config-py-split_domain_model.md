# ドメインモデル: config.py 責務分割（v3 — レビュー反映）
...
- `if __name__ == "__main__":` — 後方互換エントリポイント（config_cli.main()にデリゲート）
...
- `migrate_shell_to_toml()` — レガシーシェル設定→TOML変換
...
config_cli.py ──import──> config_migrate.py（cmd_migrate をコマンドテーブルに登録）
config_migrate.py ──import──> config.py（DEFAULTS, LIST_KEYS, config_file, legacy_config_file）
```

依存方向: config_cli.py / config_migrate.py → config.py（一方向のみ）
config.pyの__main__ガードでのconfig_cli import は実行時のみ発生し、モジュールレベルの循環にはならない。

## 呼び出しインターフェース
- `lib/config-cmd.sh`: `exec python3 "$_LIB_DIR/config_cli.py" "$@"` に変更
- `lib/config.sh`: `python3 "$JAILRUN_LIB/config_cli.py" ...` に変更
- `python3 lib/config.py <subcommand>` — 後方互換維持（__main__ガードでconfig_cli.main()にデリゲート）
