...
_cmd_add() {
  ...
  if [[ -n "$_existing" ]]; then
    echo "[token] '$_name' は既に登録済みです (${_existing:0:12}...)" >&2
    echo "[token] 更新するには 'jailrun token rotate --name $_name' を使ってください" >&2
    exit 1
  fi

  echo -n "[$_name] トークンを入力: "
  read -rs _token
  echo
...
_cmd_rotate() {
  ...
  echo "[$_name] 現在のトークン: ${_current:0:12}..."
  ...
  case "$(uname)" in
    Darwin)
      ...
      while IFS= read -r _line; do
        if [[ "$_line" == *"\"svce\"<blob>=\"${_SERVICE_PREFIX}:"* ]]; then
          local _svc
      ...
      for _svc in "${_services[@]}"; do
        local _name="${_svc#${_SERVICE_PREFIX}:}"
        local _token
        _token=$(_get_token "$_svc") || true
        if [[ -n "$_token" ]]; then
          _found=true
          echo "$_name	${_token:0:12}..."
        fi
      done
      ;;
    Linux)
