...
#
# Selects the best available backend: bwrap > systemd-run > none
...
if command -v bwrap >/dev/null 2>&1; then
  ...
  _setup_sandbox() {
    echo "[$_WRAPPER_NAME] WARN: no sandbox backend available (install bubblewrap or systemd), launching without sandbox" >&2
  }
  _build_sandbox_exec() {
    printf 'exec "$@"\n'
  }
